...
export default function UrlEncoderPage() {
  ...
  const { outputValue, error } = (() => {
    if (!input.trim()) return { outputValue: "", error: "" };
    try {
      return {
        outputValue: tab === "encode"
          ? encodeURIComponent(input)
          : decodeURIComponent(input),
        error: "",
      };
    } catch {
      return { outputValue: "", error: tab === "decode" ? "Invalid URL-encoded string." : "Encoding failed." };
    }
  })();
  ...
  return (
    <ToolPageWrapper
      ...
      description="Encode special characters in URLs or decode percent-encoded strings back to plain text, instantly."
      category="Developer"
      isFree={true}
      howItWorks={[
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-[13px] font-[500] text-[#0A0A0A] leading-none">
            {tab === "encode" ? "Raw URL / string" : "Encoded string"}
          </label>
          <textarea
            value={input}
